...
* **Architecture**: It is currently defined as a Single Page Application (SPA). Based on [App.tsx](file:///c:/Users/sanja/OneDrive/Desktop/Q%20folder/payment%20gateway%20platform/alpha_v0.1.0/shop_admin/src/App.tsx), it uses conditional rendering mapped to a state (`activeTab`) to show different views like Dashboard, Payments, Transactions, Invoices, and Customers.
* **Styling & UI**: It uses standard CSS ([App.css](file:///c:/Users/sanja/OneDrive/Desktop/Q%20folder/payment%20gateway%20platform/alpha_v0.1.0/shop_admin/src/App.css), [index.css](file:///c:/Users/sanja/OneDrive/Desktop/Q%20folder/payment%20gateway%20platform/alpha_v0.1.0/shop_admin/src/index.css)) and **Lucide React** for icons.
...
* **Database**: Configured to use **PostgreSQL** (via the `pg` package). The connection is managed in [backend/db.ts](file:///c:/Users/sanja/OneDrive/Desktop/Q%20folder/payment%20gateway%20platform/alpha_v0.1.0/shop_admin/backend/db.ts) using connection string from [.env](file:///c:/Users/sanja/OneDrive/Desktop/Q%20folder/payment%20gateway%20platform/alpha_v0.1.0/shop_admin/.env).
...
1. **The Request Initiator (Frontend UI)**: Components like [src/components/PaymentsPanel.tsx](file:///c:/Users/sanja/OneDrive/Desktop/Q%20folder/payment%20gateway%20platform/alpha_v0.1.0/shop_admin/src/components/PaymentsPanel.tsx) or [src/components/DashboardView.tsx](file:///c:/Users/sanja/OneDrive/Desktop/Q%20folder/payment%20gateway%20platform/alpha_v0.1.0/shop_admin/src/components/DashboardView.tsx) will trigger a function (e.g., inside a `useEffect` on load or clicking a submit button).
...
3. **The Entry Router (Backend Main)**: Requests reach [backend/index.ts](file:///c:/Users/sanja/OneDrive/Desktop/Q%20folder/payment%20gateway%20platform/alpha_v0.1.0/shop_admin/backend/index.ts). The Express server receives the HTTP request at `http://localhost:5000/api/...` and routes it based on the URL (e.g., `/api/payment` goes to `paymentRoutes`).
4. **The Route Handler (Backend Routes)**: Files like [backend/routes/payment.ts](file:///c:/Users/sanja/OneDrive/Desktop/Q%20folder/payment%20gateway%20platform/alpha_v0.1.0/shop_admin/backend/routes/payment.ts) map standard HTTP methods (GET, POST) to specific controller functions.
